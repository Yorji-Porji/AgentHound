...
| **Operator covering tracks** | Edit/delete audit entries after the fact | HMAC + hash-chain; `verify-audit` detects any edit, deletion, or reorder; no unsigned mode |
...
SSH public-key *filenames*, kube *context names*, npm *registry hosts*. Private
keys, tokens, and secrets are never opened for their contents. Enforced by
...
**Guarantee.** Given the per-engagement key, `verify-audit` will detect *any*
modification to the log — an edited field, a removed line, a reordered pair —
and report the first bad index. The chain is anchored at a fixed genesis hash,
so even truncating to empty and rebuilding is detectable against a retained
key/first-hash.

**Limit.** The audit log is **tamper-*evident*, not tamper-*proof*.** An
attacker who holds the key *before* writing can produce a fully valid chain of
fabricated entries. The property is: *you cannot alter history after the fact
without detection.* It is not: *you cannot lie in real time with the key.*
Protect the key accordingly (§6).
